        ...
        if (this.product) {
            ...
            if (!this.product.getHidePriceUntilCart()) {
                ...
                if (lblStockHint != null) {
                    ...
                    var freeStock = this.product.getFreeStock();
                    if (freeStock > 0) {
                        var nFreeStock = freeStock;
                        ...
                        var trueValue = nFreeStock + ' ' + ise.StringResource.getString('showproduct.aspx.47');
                        lblStockHint.innerHTML = trueValue;
                        $('#lblStockHint_' + this.id).text(trueValue);
                    }
                    else {
...
function ShowProductNotifyPriceDropPopUp(notificationCode, itemcode, productUrl, priceDropString) {
    ...
    $.ajax({
        ...
        success: function (returnValue) {
            if (returnValue) {
                if ($('#NotifyOnPriceDrop').is('div')) {
                    $('#NotifyOnPriceDrop').attr('onclick', '');
                    $('#NotifyOnPriceDrop').removeClass('divButton');
                }
                else {
                    $('#NotifyOnPriceDrop').disabled = 'disabled';
                }
                $('#NotifyOnPriceDrop').html(priceDropString);
            }
        },
...
function ShowProductOnItemAvailPopUp(notificationCode, itemcode, productUrl, notifyAvailMessagePrompt) {
    ...
    var jsonParams = JSON.stringify({ 
                        ...
                        $.ajax({
                            ...
                            success: function (returnValue) {
                                if (returnValue) {
                                    if ($('#NotifyOnItemAvail').is('div')) {
                                        $('#NotifyOnItemAvail').attr('onclick', '');
                                        $('#NotifyOnItemAvail').removeClass('divButton');
                                    }
                                    else {
                                        $('#NotifyOnItemAvail').disabled = 'disabled';
                                    }

                                    $('#NotifyOnItemAvail').html(notifyAvailMessagePrompt);
                                }
                            },
                            error: function (err, status, error) {
                                alert(err.responseText);
